...
| `g2p_install_ref`          | Git ref (branch, tag, SHA, or `refs/changes/<NN>/<num>/<ps>`) to install `gerrit_to_platform` from after the container starts. Default `main` clones upstream's main HEAD; set to an empty string to use the container's pinned PyPI release instead                                                                                                                                                                                                                                                                                                       | `main`                                                                  |
...
| `rewrite_gitreview`        | Update `.gitreview` `host` and `port` (for Bore and `None`; Tailscale leaves `port` untouched) across every replicated project so clones from the test Gerrit target the test instance. `Leave unchanged` skips the update; `Change with new commit` appends a synthetic `Chore: Rewrite` commit; `Change by amended commit` rebuilds the tip commit in place (no `Chore` commit in `git log`). For `None` (no VPN) the rewrite targets the runner's public IPv4 plus the matrix-assigned SSH port                                                         | `Change by amended commit`                                              |
| `create_missing_gitreview` | Create `.gitreview` files in replicated projects that lack one. Takes `host` and `port` from an existing `.gitreview` on another repo’s default-branch (HEAD) tip (first match wins; non-default branches are not scanned) and writes a minimal fixed-key file: `[gerrit]`, `host`, `port`, `project` (bare repo path with `.git` stripped), `defaultbranch` (from HEAD). The rewrite step, when enabled, re-targets the newly created files at the test deployment in the same run                                                                        | `false`                                                                 |
| `replicate_open_changes`   | Pull NoteDb meta refs from the source Gerrit so the deployed instance renders open changes in the UI. Adds `refs/meta/*` and `refs/cache-automerge/*` to every per-project replication remote, configures a second `<slug>-meta` remote that mirrors `refs/users/*`, `refs/groups/*`, `refs/sequences/*`, `refs/meta/*`, `refs/draft-comments/*`, `refs/starred-changes/*` into `All-Users` / `All-Projects`, and runs an online reindex + targeted cache flush after sync. See [Replicating open changes](#replicating-open-changes) below for trade-offs | `false`                                                                 |
...
### Replicating open changes

By default the deployed Gerrit mirrors `refs/heads/*`,
`refs/tags/*` and `refs/changes/*` on user projects. That is enough
for `git clone` and `git review` against the test instance, but the
Gerrit UI dashboard and `/changes/?q=status:open` will report **zero
open changes** even when the source server has hundreds. The
reasons:

1. NoteDb metadata (votes, comments, status, reviewers, hashtags)
   lives in `refs/changes/NN/CCCCCC/meta`, account identities in
   `All-Users` (`refs/users/*`, `refs/meta/external-ids`), groups
   in `All-Users` (`refs/groups/*`, `refs/meta/group-names`),
   the next change number in `All-Projects:refs/sequences/changes`,
   and project ACLs in `<project>:refs/meta/config`. None of
   those refs are in the default pull-replication scope, and
   `All-Users` / `All-Projects` are typically excluded from the
   project listing entirely.
2. Even if pull-replication delivers the refs, the Gerrit UI
   reads from the **Lucene secondary index**, not directly from
   NoteDb. Without an explicit reindex the dashboard stays empty.

The `replicate_open_changes` input fixes both. When set to `true`
the workflow:

- Extends the per-project replication refspecs with
  `+refs/meta/*:refs/meta/*` and
  `+refs/cache-automerge/*:refs/cache-automerge/*`.
- Configures a second `[remote "<slug>-meta"]` section that
  targets `All-Users` and `All-Projects` with the broader set
  (`refs/users/*`, `refs/groups/*`, `refs/sequences/*`,
  `refs/meta/*`, `refs/draft-comments/*`,
  `refs/starred-changes/*`).
- Once initial sync completes, runs an online reindex against
  every project (`POST /a/projects/<name>/index.changes`) and
  flushes the `accounts*`, `groups*`, `external_ids_map`,
  `change_notes`, `projects` and `project_list` caches so the
  replicated state becomes visible without a container restart.

**Trade-offs to be aware of:**

- Initial sync size grows materially on instances with deep
  review histories. ONAP’s `refs/changes/*` graph is comparable
  in size to the heads/tags it covers.
- The replicated `All-Projects:refs/sequences/changes` pins the
  CI Gerrit’s next-change-number to whatever the source
  reports. Do not push new changes to the test instance; any
  change uploaded there before the next replication cycle will
  collide with the source’s sequence and you will need to
  abandon it manually.
- The reindex step is asynchronous on the Gerrit side. The
  workflow returns as soon as every project has been *enqueued*;
  the UI catches up over the following seconds-to-minutes
  depending on review volume.
- Anonymous users still need read on `refs/changes/*` and
  `refs/meta/config` to see open changes in the dashboard. The
  HTTP-basic admin credentials this workflow provisions cover
  the interactive case; share the credentials with anyone who
  needs UI access during the persistent session.

```bash
# Spin up a test Gerrit that shows ONAP's open changes
remote_access: Tailscale
sync_on_startup: true
replicate_open_changes: true
persist_project_slugs: "onap"
persist_minutes: 60
```
